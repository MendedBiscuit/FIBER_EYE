...
class Impurity_Methods():
    ...
    def yuv_impurities(self, wood_mask):
        ...
        y_anom = cv2.threshold(cv2.absdiff(y_eq, int(m_y)), 2 * s_y, 255, cv2.THRESH_BINARY)[1]
        u_anom = cv2.threshold(cv2.absdiff(u_eq, int(m_u)), 2 * s_u, 255, cv2.THRESH_BINARY)[1]
        v_anom = cv2.threshold(cv2.absdiff(v_eq, int(m_v)), 2 * s_v, 255, cv2.THRESH_BINARY)[1]
    ...
    def hsv_impurities(self, wood_mask):
        ...
        h_anom = cv2.threshold(cv2.absdiff(h_masked, int(m_h)), 2 * s_h, 255, cv2.THRESH_BINARY)[1]
        s_anom = cv2.threshold(cv2.absdiff(s_eq, int(m_s)), 2 * s_s, 255, cv2.THRESH_BINARY)[1]
        v_anom = cv2.threshold(cv2.absdiff(v_eq, int(m_v)), 2 * s_v, 255, cv2.THRESH_BINARY)[1]
